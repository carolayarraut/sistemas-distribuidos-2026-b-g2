...
|--------|-----------|---------------------|
| API Gateway | Sí | Punto único de entrada — [ADR-XXX] |
| Database per Service | Sí | Independencia de datos entre servicios |
| Circuit Breaker | Sí | Resiliencia en llamadas síncronas |
| Saga (coreografiada) | Sí | Transacciones distribuidas via eventos |
| Outbox Pattern | Sí | Garantía de publicación de eventos |
| CQRS | No | Complejidad no justificada en la fase actual |
| Event Sourcing | No | No requerido en MVP |
